...
class ZenodoService(BaseService):
    ...
    def create_new_deposition(self, dataset: DataSet) -> dict:
        ...
        metadata = {
            ...
            "keywords": (
                ["carhub"] if not dataset.ds_meta_data.tags else dataset.ds_meta_data.tags.split(", ") + ["carhub"]
            ),
            "access_right": "open",
            "license": "CC-BY-4.0",
        }
    ...
    def upload_file(self, dataset: DataSet, deposition_id: int, feature_model: FeatureModel, user=None) -> dict:
        ...
        data = {"name": car_filename}
        user_id = current_user.id if user is None else user.id
        file_path = os.path.join(uploads_folder_name(), f"user_{str(user_id)}", f"dataset_{dataset.id}/", car_filename)
        files = {"file": open(file_path, "rb")}
