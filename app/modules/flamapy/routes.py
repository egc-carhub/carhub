...
@flamapy_bp.route("/flamapy/check_car/<int:file_id>", methods=["GET"])
def check_car(file_id):
    class CustomErrorListener(ErrorListener):
        ...
        def syntaxError(self, recognizer, offendingSymbol, line, column, msg, e):
            if "\\t" in msg:
                warning_message = (
                    f"The CAR has the following warning that prevents reading it: " f"Line {line}:{column} - {msg}"
                )
                print(warning_message)
                self.errors.append(warning_message)
            else:
                error_message = (
                    f"The CAR has the following error that prevents reading it: " f"Line {line}:{column} - {msg}"
                )
